...
# @mock-exempt: httpx.AsyncClient is mocked because it represents an external HTTP
# service boundary. We must not make real network calls in unit tests.
"""
...
class TestFetchAndParse:
    async def test_fetch_html_success(self):
        ...
        with patch("sat.ingestion.fetcher.httpx.AsyncClient", return_value=mock_client):
            doc = await fetch_and_parse("https://example.com/page")

        assert doc.source_name == "https://example.com/page"
    ...
    async def test_fetch_404_returns_warning(self):
        ...
        with patch("sat.ingestion.fetcher.httpx.AsyncClient", return_value=mock_client):
            doc = await fetch_and_parse("https://example.com/missing")
    ...
    async def test_fetch_timeout_returns_warning(self):
        ...
        with patch("sat.ingestion.fetcher.httpx.AsyncClient", return_value=mock_client):
            doc = await fetch_and_parse("https://slow.example.com/", timeout=1.0)
